...
- 2026-06-17: Fixed the Phase 4D Cookie logging blocker. The active Nginx `site_total` log format now preserves the `cookie` JSON field but writes an empty string instead of `$http_cookie`; backup is `/etc/framespark/backups/nginx-site-total-no-cookie/20260617T082123Z`. `nginx -t` passed, Nginx reloaded once, homepage and `/diagnosis/` stayed `200`, `/diagnosis/beta/` no-auth stayed `401`, Diagnosis service stayed `active/running/enabled`, `NRestarts=0`, ready/health OK, `8788` loopback-only, provider/metadata/review stayed `1 / 2 / 0`, and no invite-code route/static deployment, real code, Diagnosis POST, AI call or B4 T0 occurred.
