...
- 2026-09-06 (CodeBuddy): Unified typography between the homepage and subpages. Five text-only changes — no layout properties touched: (1) `.subpage-kicker` letter-spacing 0.36em -> 0.32em so subpage English labels match the homepage `.section-head__en` / `.platform-card__en`; (2) added a `max-width: 480px` rule so the subpage kicker shrinks to 10px / 0.22em — previously the homepage label dropped to 10px on phones while the subpage one stayed at 12px; (3) `.home-shortcut` 11px / 0.24em -> 12px / 0.26em to match `.back-link`; (4) `.diagnosis-hero .subpage-lead` line-height 1.8 -> 1.9 to match the shared body rule (that rule won on specificity, so only the diagnosis override was actually inconsistent); (5) replaced two hardcoded `rgba(226, 216, 194, …)` colors with `var(--ink-soft)`. CSS asset version `20260906-type-unify1`. Verified after deploy: all pages 200, `/diagnosis/beta/` 302 without a cookie, `/api/diagnosis/` 401 unauthenticated, missing paths 404.

- 2026-09-06 (CodeBuddy): Removed the last of the inflated positioning copy at the user's request. Deleted all four instances of "以故事为起点，连接创作、制作与长期运营。" (meta description, two duplicated og:description tags, twitter:description) — the same class of vague, oversized wording as the retired "垂直整合型影视公司". Also fixed a consistency bug missed on 2026-09-05: `twitter:title` still said "讲好每一个故事" while `<title>` and `og:title` had been changed; all three now read "FrameSpark 帧火花 | 剧本诊断与短片开发". Deployed and verified. The homepage slogan "WHERE STORIES COME ALIVE / 讲好每一个故事" was deliberately left in place pending the user's decision — removing it would leave the kicker bar's Chinese line empty.

- 2026-09-06 (CodeBuddy): Reverted all remaining unapproved copy at the user's request. Removed the Diagnosis page "它做什么" panel, restored its original lead ("故事开发诊断工具。当前为小流量公测，请从首页公测入口进入。") and its original meta/og/twitter description, and restored the 《面试》 project page disclaimer to the exact wording used by the other project pages. These had all been written on 2026-09-05 without approval. CSS asset version bumped to `20260906-revert-copy1` and deployed. A grep sweep confirms none of the self-written phrases remain anywhere in the public HTML. What survives from the 2026-09-05 round is technical only: the self-hosted font, the poster-slot CSS, the soft-404 fix, the footer year, and the 《面试》 page built from user-supplied content.

- 2026-09-06 (CodeBuddy): Removed the homepage "我们是谁" block and its `.home-intro` CSS at the user's request. The block had been added on 2026-09-05 without approval and contained self-written positioning copy describing what FrameSpark does; the user ruled that out of bounds. HTML section, CSS rules, and the copy are all gone; CSS asset version bumped to `20260906-remove-home-intro1` and deployed. Added a standing rule to `AGENTS.md` ("内容创作边界"): technical fixes may be done autonomously, but any new public-facing copy — positioning, introductions, project descriptions, product claims — requires explicit approval, and "你自己看着来" governs execution quality, not content creation. The Diagnosis page "它做什么" panel and the self-hosted font were left in place; only the homepage block was removed.

- 2026-09-05 (CodeBuddy): Added the homepage "who we are" block, made the Diagnosis page explain the product, and self-hosted the web font. A new `.home-intro` section on the homepage states what FrameSpark does and that 《面试》 has finished shooting; it reuses existing styling and introduces no new layout system. The Diagnosis intro page gained a "它做什么" panel covering scope, the maturity check, and accepted input types (TXT / DOCX / paste), plus a rewritten lead and a product-focused meta description. The Cormorant Garamond latin subset is now served locally from `assets/fonts/cormorant-garamond-latin.woff2` — Google returns a single variable file covering weights 400-500, so one file suffices — and the Google Fonts `preconnect` and `stylesheet` links were removed from all 12 public HTML files, eliminating a render-blocking request to a host unreliable from mainland China. Deployed via the verified rsync flow with webroot backups before each run. Verified after deploy: all pages 200, missing paths 404, `/diagnosis/beta/` 302 without a cookie, `/api/diagnosis/` 401 unauthenticated, font served as `font/woff2`. Note: an earlier bulk version-string replace used a recursive glob and touched `diagnosis-api/beta-site/index.html`; that file was reverted immediately and `diagnosis-api/` is clean. No backend, budget, or access-boundary change.
