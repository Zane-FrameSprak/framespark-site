...
- Poster slot is ready but intentionally empty pending real artwork. Spec: `.project-detail__poster` is `aspect-ratio: 3 / 4`, rendered at `min(320px, 72vw)` wide (desktop 320x427 CSS px). Supply art at **3:4 portrait, minimum 960x1280 px** (640x853 absolute floor), WebP preferred, JPG acceptable, keep under 200 KB. To install, add one line inside the existing `.project-detail__poster` div in `projects/interview.html`: `<img src="../assets/brand/interview-poster.webp" alt="《面试》海报">`. Nothing else changes — CSS already applies `object-fit: cover` so the art is cropped rather than distorted, hides the placeholder number via `:has(img)`, hides the bottom gold rule, and keeps the inner frame above the image. Never use AI-generated art for this slot: the poster must come from real footage from the shoot, or it misrepresents the film.
...
- 《面试》 status must stay consistent everywhere it appears. If it finishes post-production or the release plan changes, update every mention together — they must never disagree.
- Typography baseline (established 2026-09-06 — keep it consistent): English kickers 12px / letter-spacing 0.32em / `var(--gold)`, shrinking to 10px / 0.22em under 480px; body text 15px / line-height 1.9 / `var(--ink-soft)`; meta labels 11px / 0.28em / gold; return links (both `.back-link` and `.home-shortcut`) 12px / 0.26em / gold; Chinese titles `var(--font-serif-cn)` with 0.08em. Always use the colour custom properties, never hardcoded rgba.
- Note when changing typography: `.diagnosis-hero .subpage-lead` has higher specificity than the shared body rule, so it must be updated alongside it or it silently drifts.
- Do not add public-facing copy without explicit approval. Fixing, cleaning, and deploying are fine; writing new positioning, introduction, or product copy is not. See the content boundary rule in `AGENTS.md`.
- The Diagnosis intro page was reverted to its original wording on 2026-09-06; an unapproved "它做什么" panel and rewritten lead were removed. It currently only points to the homepage beta entry. Improving it is welcome, but the copy must be supplied or approved by the user first.
