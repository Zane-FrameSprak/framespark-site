...
- Phase 4D.1 read-only precheck is complete and blocked on Nginx logging: the effective `site_total` log format records `$http_cookie`, which would risk logging invite-code session cookies after Cookie auth is exposed. Use `docs/diagnosis/DIAGNOSIS_BETA_PHASE4D_READONLY_PRECHECK_2026-06-17.md` as the evidence. Do not proceed to homepage/static deployment, Nginx session-auth, reload, real tester codes, Diagnosis POST, AI, B4 T0 or invitations until a separate no-cookie logging fix is planned and approved.
