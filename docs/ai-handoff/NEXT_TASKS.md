...
- Diagnosis Beta invite-code launch is live. Production Diagnosis API `current` is `8e089c5bcf68086c787a3cafc58ba358d92e1ee1`; `previous` is `e16d6997c5dc4c08671c7c2f8d66d0dd989e90bf`. Homepage invite-code UI is deployed, `/diagnosis/beta/` without a valid Cookie redirects to `/#diagnosis-beta-entry`, and exact Beta static/API routes are protected by backend Cookie session validation. `ENABLE_BETA_CODE_ACCESS=true`; service is `active/running/enabled`, `NRestarts=0`, ready/health OK, and `8788` remains loopback-only.
- Repository-side public beta preparation is complete but not deployed. The next step is a controlled public-beta deployment from the new commit: prebuilt backend artifact, env switch to `ENABLE_PUBLIC_BETA_ACCESS=true`, exact Nginx exposure for `/api/beta-access/public-session`, updated static homepage/Beta assets, no-AI boundary checks, and only then a separately authorized one-call fictional real-AI smoke.
...
- Continue to avoid Cookie/Authorization logging. The deployed Phase 4D route-level timing log is narrow, but the panel-managed global `site_total` log format has previously reverted to `$http_cookie`; inspect effective Nginx logging before any broader invite-code or diagnosis expansion.
