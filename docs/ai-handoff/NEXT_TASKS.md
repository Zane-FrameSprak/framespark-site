...
- A later Phase 3 retry must use a Linux same-architecture prebuilt diagnosis-api artifact containing production `node_modules`, verified by `npm audit --omit=dev`, native `better-sqlite3` load, and `DIAGNOSIS_DATA_DIR=<isolated> npm run test:server-release` before upload. Do not run `test:beta-access-frontend` inside a diagnosis-api-only server release; keep it for complete-repository and Phase 4 static deployment validation.
