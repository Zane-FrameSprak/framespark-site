...
- Diagnosis public beta is live. Production Diagnosis API `current` is `535640213756e07a4c144f1ea4eb9df98b2e305b`; `previous` is `ce440d7a92a7e6141915c3bfbb1e3277bb8fbb82`. Homepage public beta UI is deployed, `/diagnosis/beta/` without a valid Cookie redirects to `/#diagnosis-beta-entry`, and exact Beta static/API routes are protected by backend Cookie session validation. `ENABLE_PUBLIC_BETA_ACCESS=true`, `ENABLE_BETA_CODE_ACCESS=false`; service is `active/running/enabled`, `NRestarts=0`, ready/health OK, and `8788` remains loopback-only.
- Repository-side Diagnosis input admission now uses `tiktoken` with `cl100k_base` and `MAX_INPUT_TOKENS=50000` instead of the old character-count cap. Keep the release/deploy path separate: no production deployment has occurred for this token-limit change yet. When deploying it, preserve the intended request order of file-size/upload limit -> token limit -> daily counters, and verify over-token requests return `TEXT_TOO_LONG` without daily counter consumption or provider calls.
...
- Initial public-beta caps are fixed at account/session `1`, IP `3`, global diagnoses `5`, provider daily `30`, provider tokens `5,000,000` per UTC day, concurrency `1`, and provider calls per diagnosis `5`. Do not raise these limits before the first observation window.
...
- Continue to avoid Cookie/Authorization logging. The public-beta switch set the panel-managed global `site_total` cookie field to an empty string instead of `$http_cookie`; re-check effective Nginx logging after any panel save, Nginx edit, or route expansion.
