...
- Phase 4D.1 read-only precheck found a Cookie logging blocker, and `docs/diagnosis/DIAGNOSIS_BETA_NGINX_NO_COOKIE_LOG_FIX_2026-06-17.md` records the completed fix: `site_total` now keeps the `cookie` field but writes an empty string instead of `$http_cookie`. Next Phase 4D work can continue with static artifact preparation and the Nginx invite-code/session boundary, but still must not create real tester codes, execute Diagnosis POST, call AI, start B4 T0 or invite testers before the relevant phase is approved.
