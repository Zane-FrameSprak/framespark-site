...
### 部署策略（2026-09-06 用户明确）

- **纯视觉 / 内容改动**：可直接 push 并部署官网。范围包括文案、配色、间距、字号、字体、图片、静态页面。
- **涉及功能的改动**：必须等功能完整做好并验证通过后再部署。**禁止把半成品推上生产**，避免官网出现使用上的问题。
- 判断不清时按"涉及功能"处理：先在本地完成并验证，确认可用后一次性上线。
...
用户明确的长期规则。违反一次即视为严重错误。

- **未经用户明确允许，不得新增任何面向公众的内容**：文案、介绍段落、区块、栏目、页面。
- 技术性改动可自主执行：修 bug、清理死代码、性能优化、配置修复、文档同步、部署。
- **新增"内容"必须先经用户确认**，尤其是以下对外表态类文案：
  - 公司定位 / 我们是谁 / 品牌介绍
  - 项目介绍与故事简介
  - 团队与主创介绍
  - 产品功能描述
- 分不清算"修复"还是"新增内容"时，**按新增内容处理，先问再做**。
- 用户说"你自己看着来""按大厂做法来"，指的是**执行方式和质量标准**，不等于授权创作新内容。

历史教训：2026-09-05 未经确认新增首页"我们是谁"区块（含自拟的公司定位文案），2026-09-06 按用户要求移除。
