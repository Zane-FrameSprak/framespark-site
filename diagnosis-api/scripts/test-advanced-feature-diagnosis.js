 ...
 *
 * 用法：
 *   node scripts/test-advanced-feature-diagnosis.js          # 完整测试（需配置 DEEPSEEK_API_KEY）
 *   node scripts/test-advanced-feature-diagnosis.js --no-ai  # 仅验证 prompt 结构，不调用 AI
 */
...
const ALLOWED_PREFIXES = ['建议继续打磨', '需要结构性重写', '可进入下一阶段评估', '暂不适合继续深化'];
// 长片进阶允许简要提示风险信号，但以下绝对禁止出现
...
// ── 加载模块 ────────────────────────────────────────────────────
const { ADVANCED_FEATURE_CASES } = await import('../tests/fixtures/advanced-feature-cases.js');
const { buildAdvancedFeatureDiagnosisMessages } = await import('../src/prompts/advancedFeatureDiagnosis.js');
...
function validateAiResponse(report, testCase) {
  ...
  if (expectedNextStepPrefixes?.length > 0) {
    ...
    if (!matchesExpected) {
      warnings.push(`nextStep 期望前缀为 [${expectedNextStepPrefixes.join('/')}]，实际：${nextStep.slice(0, 50)}`);
    }
  }
...
// ── AI 调用（不挂入正式流程，测试专用）──────────────────────────
async function callAdvancedFeatureDiagnosis(text) {
  const stats = makeStats(text);
  const messages = buildAdvancedFeatureDiagnosisMessages({ text, stats, source: { filename: 'test' } });
...
async function runCase(testCase, index, total) {
  ...
  try {
    ...
    const messages = buildAdvancedFeatureDiagnosisMessages({ text: inputText, stats, source: { filename: 'test' } });
    ...
    result.promptCheck.errors = [`buildAdvancedFeatureDiagnosisMessages 抛出异常：${err.message}`];
  }
  ...
  try {
    const report = await callAdvancedFeatureDiagnosis(inputText);
...
// ── 主流程 ───────────────────────────────────────────────────────
async function main() {
  const total = ADVANCED_FEATURE_CASES.length;
  const startTime = Date.now();

  console.log(`\n${c.bold}帧火花长片进阶诊断稳定性测试${c.reset}`);
  console.log(`用例总数：${total}  AI 调用：${NO_AI ? '关闭（--no-ai）' : aiAvailable ? '开启' : '未配置（跳过）'}`);
  console.log('─'.repeat(52));

  const results = [];
  for (let i = 0; i < ADVANCED_FEATURE_CASES.length; i++) {
    const result = await runCase(ADVANCED_FEATURE_CASES[i], i, total);
    results.push(result);
    if (aiAvailable && i < ADVANCED_FEATURE_CASES.length - 1) await sleep(CALL_DELAY_MS);
  }
  ...
  const outDir  = path.join(ROOT, '..', 'test-results', 'advanced-feature');
  ...
  try {
    ...
    console.log(`${c.gray}结果已保存至：test-results/advanced-feature/${timestamp}.json${c.reset}\n`);
  } catch (err) {
