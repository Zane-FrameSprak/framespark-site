 ...
 *
 * 用法：
 *   node scripts/test-advanced-short-diagnosis.js          # 完整测试（需配置 DEEPSEEK_API_KEY）
 *   node scripts/test-advanced-short-diagnosis.js --no-ai  # 仅验证 prompt 结构，不调用 AI
 */
...
// ── 加载模块 ────────────────────────────────────────────────────
const { ADVANCED_SHORT_CASES } = await import('../tests/fixtures/advanced-short-cases.js');
const { buildAdvancedShortDiagnosisMessages } = await import('../src/prompts/advancedShortDiagnosis.js');
...
// ── AI 调用（不挂入正式流程，测试专用）──────────────────────────
async function callAdvancedDiagnosis(text) {
  const stats = makeStats(text);
  const messages = buildAdvancedShortDiagnosisMessages({ text, stats, source: { filename: 'test' } });
...
async function runCase(testCase, index, total) {
  ...
  try {
    ...
    const messages = buildAdvancedShortDiagnosisMessages({ text: inputText, stats, source: { filename: 'test' } });
    ...
    result.promptCheck.errors = [`buildAdvancedShortDiagnosisMessages 抛出异常：${err.message}`];
  }
  ...
  try {
    const report = await callAdvancedDiagnosis(inputText);
...
// ── 主流程 ───────────────────────────────────────────────────────
async function main() {
  const total = ADVANCED_SHORT_CASES.length;
  const startTime = Date.now();

  console.log(`\n${c.bold}帧火花短片进阶诊断稳定性测试${c.reset}`);
  console.log(`用例总数：${total}  AI 调用：${NO_AI ? '关闭（--no-ai）' : aiAvailable ? '开启' : '未配置（跳过）'}`);
  console.log('─'.repeat(52));

  const results = [];
  for (let i = 0; i < ADVANCED_SHORT_CASES.length; i++) {
    const result = await runCase(ADVANCED_SHORT_CASES[i], i, total);
    results.push(result);
    if (aiAvailable && i < ADVANCED_SHORT_CASES.length - 1) await sleep(CALL_DELAY_MS);
  }
  ...
  const outDir  = path.join(ROOT, '..', 'test-results', 'advanced-short');
  ...
  try {
    ...
    console.log(`${c.gray}结果已保存至：test-results/advanced-short/${timestamp}.json${c.reset}\n`);
  } catch (err) {
