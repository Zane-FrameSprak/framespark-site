...
import { BETA_IDENTITY_HEADER, betaIdentityForCode } from '../services/betaAccessService.js';
...
export function createBetaAccessRouters(options) {
  ...
  internalRouter.get('/validate', (req, res) => {
    ...
    res.set(BETA_IDENTITY_HEADER, betaIdentityForCode(session.identityId));
    res.status(204).end();
  });
...
function strictOrigin(allowedOrigins) {
  return function betaAccessOriginGuard(req, res, next) {
    ...
    res.status(403).json({
      ok: false,
      error: { code: 'ORIGIN_NOT_ALLOWED', message: '当前请求来源不允许使用诊断内测。' }
    });
  };
}
